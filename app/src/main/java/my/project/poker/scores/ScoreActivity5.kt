...
class ScoreActivity5 : AppCompatActivity() {
    ...
    private fun loadText(context: Context) {
        ...
        if (sPref.getString("pl1", "")?.isNotEmpty()!!) {
            ...
            val header = DataTableHeader.Builder()
                ...
                .item(getString(R.string.gamer5), 2).build()
            val rows = ArrayList<DataTableRow>()
            if (pl1.isNotEmpty() && pl2.isNotEmpty() && pl3.isNotEmpty() && pl4.isNotEmpty() && pl5.isNotEmpty()) {
                for (i in pl1.indices) {
                    val row =
                        if (pl1[i] == "0" && pl2[i] == "0" && pl3[i] == "0" && pl4[i] == "0" && pl5[i] == "0") {
