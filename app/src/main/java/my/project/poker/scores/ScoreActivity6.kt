...
class ScoreActivity6 : AppCompatActivity() {
    ...
    private fun loadText(context: Context) {
        ...
        if (sPref.getString("pl1", "")?.isNotEmpty()!!) {
            ...
            val pl4: ArrayList<String> = Gson().fromJson(
                sPref.getString("pl4", ""),
                object : TypeToken<ArrayList<String?>?>() {}.type
            )
            val pl5: ArrayList<String> = Gson().fromJson(
                sPref.getString("pl5", ""),
                object : TypeToken<ArrayList<String?>?>() {}.type
            )
            val pl6: ArrayList<String> = Gson().fromJson(
                sPref.getString("pl6", ""),
                object : TypeToken<ArrayList<String?>?>() {}.type
            )
            ...
            val header = DataTableHeader.Builder()
                ...
                .item(getString(R.string.gamer3), 2)
                .item(getString(R.string.gamer4), 2)
                .item(getString(R.string.gamer5), 2)
                .item(getString(R.string.gamer6), 2).build()
            val rows = ArrayList<DataTableRow>()
            if (pl1.isNotEmpty() && pl2.isNotEmpty() && pl3.isNotEmpty() && pl4.isNotEmpty() && pl5.isNotEmpty() && pl6.isNotEmpty()) {
                for (i in pl1.indices) {
                    val row =
                        if (pl1[i] == "0" && pl2[i] == "0" && pl3[i] == "0" && pl4[i] == "0" && pl5[i] == "0" && pl6[i] == "0") {
